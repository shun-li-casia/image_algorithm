...
// TODO: camera param
constexpr float fx = 6293.079229545598 * 0.5;
constexpr float fy = 6293.079229545598 * 0.5;
constexpr float cx = 4782.385009765625 * 0.5;
constexpr float cy = 3186.627929687 * 0.5;
constexpr float b = 0.1824021461936596;
...
int main(int argc, char** argv) {
  ...
  // par.add<std::string>("rect_cfg", 0, "rectification config", true);
  par.parse_check(argc, argv);

  const string lfile = par.get<string>("l_img");
  const string rfile = par.get<string>("r_img");
  // const string rect_cfg = par.get<string>("rect_cfg");
  ...
  while (true) {
    ...
    disp.convertTo(disparity, CV_32F, 1.0 / 16.0);
    for (int i = 0; i < left.rows; i++) {
      for (int j = 0; j < left.cols; j++) {
        float d = disparity.at<float>(i, j);
        if (d <= 0 || d > 128) continue;
        float depth = fx * b / d;
        pcl::PointXYZ p;
        p.x = (j - cx) * depth / fx;
        p.y = (i - cy) * depth / fy;
